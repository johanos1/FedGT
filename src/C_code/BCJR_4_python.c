...
static double noisy_test_outcome(int state, const uint8_t *test_vec)
{
    ...
    for(int i=0; i<N-K; i++)
    {
      probability *= Channel_model[test_vec[i]][state_vec[i]]; //diff from Matlab
    }
    free(state_vec);
    return probability;
}
...
static void build_trellis ()
{
	...
	for (t=0; t<=N; t++) {
      ...
      for(int lomi=0; lomi<num_of_from_states; lomi++)
        fromstate[t][curstate][lomi] = NO_STATE;
		}
	}
	...
	for (t=0; t<N; t++) {
        ...
        for(int lomi=1; lomi<num_of_from_states; lomi++)
        {
            if (!VALID_STATE(fromstate[t+1][nextstate][lomi])) {
              fromstate[t+1][nextstate][lomi] = curstate;
              break;
            }
        }
			}
	...
	for (t=1; t<N; t++) {
		...
		if (nextlabel != numstates[t]) { //makes sense
		...
		for (newlabel=0; newlabel<numstates[t]; newlabel++) {
          ...
          {
            int cimi = -1;
            for(int lomi=1; lomi<num_of_from_states; lomi++) //find the old label place
                {
                  if(fromstate[t+1][tostate[t][newlabel][1]][lomi] == oldlabel)
                  {
                    cimi = lomi;
                    fromstate[t+1][tostate[t][newlabel][1]][cimi] = newlabel;
                    break;
                  }
                }
            if (cimi == -1)
            {  printf("Cimi should have been updated!\n");
              exit(-2);
			}
          }
        ...
        for(int lomi=1; lomi<num_of_from_states; lomi++)
        {
        fromstate[t][newlabel][lomi] = fromstate[t][oldlabel][lomi];
				if (VALID_STATE(fromstate[t][newlabel][lomi]))
				    tostate[t-1][fromstate[t][newlabel][lomi]][1] = newlabel;
				fromstate[t][oldlabel][lomi] = NO_STATE; //needs to be fixed!
		}
      }
		}
	}
...
void map_decoder (const double *llri, double *llro, uint8_t *dec, double threshold_dec, const uint8_t *test_vec)
{
  ...
  for (t=1; t<N; t++) {
      ...
      for(int lomi=1; lomi<num_of_from_states; lomi++)
      {
        prevstate = fromstate[t][curstate][lomi];
        if (VALID_STATE(prevstate))
  ...
  for(int statusija =0; statusija < maxstates; statusija++)
  {
	...
	//printf("\nStatusi: %d, prob: %f", statusija, temp_testi);
	...
	for (t=0; t<N; t++) {
		...
		for (curstate=0; curstate<numstates[t]; curstate++) {
			nextstate = tostate[t][curstate][0];
			if (VALID_STATE(nextstate))
        log_app[0] = maxstar(log_app[0],log_alpha[t][curstate]+llri[t]+log_beta[t+1][nextstate]); //check if this is fine!
			nextstate = tostate[t][curstate][1];
			if (VALID_STATE(nextstate))
		...
		//llro[t] -= llri[t]; // This is extrinsic info, do we need this?
	}
}
/* End of map_decoder*/
